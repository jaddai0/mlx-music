"""
ACE-Step v1.5 LoRA Trainer.

Provides a complete training loop for ACE-Step v1.5 with:
- LoRA fine-tuning of DiT attention and MLP layers
- Flow matching loss with log-normal timestep sampling
- EMA weight averaging
- Learning rate scheduling
- Gradient accumulation
- Checkpointing with security-validated paths

Example:
    from mlx_music.training.ace_step_v15 import ACEStepV15Trainer, TrainingConfigV15

    config = TrainingConfigV15(
        output_dir="./v15_checkpoints",
        learning_rate=1e-4,
        num_epochs=10,
        lora_rank=64,
    )

    trainer = ACEStepV15Trainer(model=model, config=config)
...
from .dataset import AudioBatchV15, AudioDatasetV15
from .lora_layers import apply_lora_to_v15_model
from .loss import ACEStepV15Loss

logger = logging.getLogger(__name__)


def _materialize(*args) -> None:
    """Force MLX lazy evaluation to materialize computation graphs."""
    mx.eval(*args)


@dataclass
class TrainingConfigV15:
    """Configuration for ACE-Step v1.5 training."""

    # Output
    output_dir: Path = field(default_factory=lambda: Path("./ace_step_v15_checkpoints"))
    experiment_name: str = "ace_step_v15_lora"
...
class ACEStepV15Trainer:
    """Trainer for ACE-Step v1.5 LoRA fine-tuning.

    Handles the complete training loop including:
    - LoRA layer injection into DiT attention/MLP
    ...
    def __init__(
        ...
        Args:
            model: ACE-Step v1.5 AceStepConditionGenerationModel
            config: Training configuration
        """
        self.model = model
        self.config = config
        self.state = TrainingStateV15()
        ...
        if config.use_lora:
            num_lora = apply_lora_to_v15_model(
                model,
                rank=config.lora_rank,
                alpha=config.lora_alpha,
                dropout=config.lora_dropout,
                include_mlp=config.lora_include_mlp,
            )
            # Freeze all base weights, unfreeze only LoRA parameters
            model.freeze()
            model.apply_to_modules(
                lambda k, m: m.unfreeze(keys=["lora_A", "lora_B"])
                if hasattr(m, "lora_A")
                else None
            )
            logger.info(
                f"Applied LoRA to {num_lora} layers "
                f"(rank={config.lora_rank}, include_mlp={config.lora_include_mlp})"
            )
    ...
    def _prepare_conditioning(
        ...
        """Prepare conditioning inputs from a batch.

        For training, we need encoder_hidden_states and context_latents.
        Since the full condition preparation requires the encoder,
        we pass pre-computed embeddings through a simplified path.
        """
        B = batch.encoded_latents.shape[0]
        T = batch.encoded_latents.shape[1]
        dtype = mx.bfloat16 if self.config.use_bf16 else mx.float32

        # Use text embeddings as encoder hidden states
        # In full pipeline, this goes through the condition encoder
        encoder_hidden_states = batch.text_embeddings.astype(dtype)
        encoder_attention_mask = mx.ones(
            (B, encoder_hidden_states.shape[1]), dtype=dtype
        )

        # Context latents = src_latents (silence) + chunk_masks
        # During training, src_latents are the clean latents themselves
        # and chunk_masks are all ones (fully visible)
        src_latents = batch.encoded_latents.astype(dtype)
        chunk_masks = mx.ones_like(src_latents)
        context_latents = mx.concatenate([src_latents, chunk_masks], axis=-1)
    ...
    def _compute_loss(self, batch: AudioBatchV15) -> mx.array:
        """Compute flow matching loss for a v1.5 batch."""
        ...
        loss, _ = ACEStepV15Loss.forward(
            ...
            mu=self.config.timestep_mu,
            sigma=self.config.timestep_sigma,
            data_proportion=self.config.data_proportion,
            rng=batch.rng,
        )
        return loss

    def _train_step(self, batch: AudioBatchV15) -> Dict[str, float]:
        ...
        if accumulated is not None:
            ...
            _materialize(self.model.parameters())
    ...
    def train(
        self,
        dataset: AudioDatasetV15,
        val_dataset: Optional[AudioDatasetV15] = None,
        ...
        logger.info(f"Starting v1.5 training: {total_steps} steps, {self.config.num_epochs} epochs")
        logger.info(
            f"Effective batch size: "
            f"{self.config.batch_size * self.config.gradient_accumulation_steps}"
        )
        logger.info(
            f"Flow matching: mu={self.config.timestep_mu}, "
            f"sigma={self.config.timestep_sigma}, "
            f"data_proportion={self.config.data_proportion}"
        )
    ...
    def _validate(self, dataset: AudioDatasetV15) -> float:
    ...
    def load_checkpoint(self, checkpoint_dir: Path | str) -> None:
        ...
        if lora_path.exists():
            from mlx_music.training.ace_step.lora_layers import load_lora_weights
