"""
Gradient Accumulation for mlx-music Training.
...
Based on mflux Qwen training gradient accumulator implementation.

IMPORTANT: Uses mlx.core.eval() to prevent lazy evaluation graph explosion.
...
class GradientAccumulator:
    ...
    CRITICAL: Forces evaluation with mlx.core.eval() after each accumulation
    ...
    def _force_evaluate(self, tree: dict[str, Any]) -> None:
        """Force MLX lazy evaluation to materialize computation graph."""
        # mx.eval triggers lazy evaluation in MLX - this is NOT Python's eval()
        mx.eval(tree)
    ...
    def accumulate(self, grads: dict[str, Any]) -> dict[str, Any] | None:
        ...
        self._force_evaluate(self._accumulated)
